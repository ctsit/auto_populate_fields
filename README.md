...
## Installation
- Clone this repo into `<redcap-root>/modules/auto_populate_fields_v<version>`.
...
Though the data is erased, the null value written to the record is still a legitimate value that could be copied to the next event filled in. This will only occur in new data entry if the last event in the sequence was deleted. Note that "sequence" here refers to chronological order.
