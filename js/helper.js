$(document).ready(function() {
    ...
    $('body').on('dialogopen', function(event, ui) {
        ...
        var texts = {
            '@DEFAULT-FROM-FIELD': 'Sets a fields\'s default value from an existing field on the same form. This is useful when using hidden fields as source for visible fields, e.g. @DEFAULT-FROM-FIELD=\'hidden_first_name\'.',
        ...
        var $prev_action_tag = $default_action_tag;
        $.each(texts, function(tag_name, descr) {
            ...
            $new_action_tag.children('td').filter(isDefaultLabelColumn).text(tag_name);

            // Column 3: updating action tag description.
            $new_action_tag.children('td').last().text(descr);

            // Placing new action tag.
            $new_action_tag.insertAfter($prev_action_tag);
            $prev_action_tag = $new_action_tag;
        });
    });
});
